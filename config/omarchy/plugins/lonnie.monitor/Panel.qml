...
Panel {
  ...
  // ---- Auto-brightness (arch-on-pro's ALS daemon) ----
  // "Live" is inferred from freshness of the daemon's status file rather
  // than a separate `systemctl is-active` poll — the daemon writes it about
  // once a second while actually running, and unlinks it on a clean stop.
  property int autoBrightnessLux: -1
  property bool autoBrightnessPaused: false
  property real autoBrightnessTs: 0
  property bool autoBrightnessLive: false
  property bool autoBrightnessBusy: false

  // What the toggle actually represents: is it doing its job right now.
  // "Live but paused" (you dragged the slider yourself) reads as
  // disengaged — the toggle is a decisive on/off, not a raw process-alive
  // flag, so clicking it while paused should seize control back rather
  // than stop a daemon that's still technically running.
  readonly property bool autoBrightnessEngaged: autoBrightnessLive && !autoBrightnessPaused

  function refreshAutoBrightnessLive() {
    autoBrightnessLive = autoBrightnessTs > 0 && (Date.now() / 1000 - autoBrightnessTs) < 1.5
  }

  function toggleAutoBrightness() {
    if (autoBrightnessBusy) return
    if (autoBrightnessEngaged) {
      // Stop is deterministic — flip the switch now instead of waiting on
      // the process + a file re-read to confirm what we already know.
      autoBrightnessLive = false
      autoBrightnessTs = 0
      autoBrightnessToggleProc.command = ["systemctl", "--user", "stop", "auto-brightness.service"]
    } else if (autoBrightnessLive) {
      // Running but paused on a manual override — force it to resume now
      // instead of waiting for the lux to swing 75%. Also deterministic.
      autoBrightnessPaused = false
      autoBrightnessToggleProc.command = ["systemctl", "--user", "kill", "-s", "SIGUSR1", "auto-brightness.service"]
    } else {
      // Start genuinely has to wait for the daemon to come up, so this is
      // the one case worth a busy/"Working…" beat.
      autoBrightnessBusy = true
      autoBrightnessToggleProc.command = ["systemctl", "--user", "start", "auto-brightness.service"]
    }
  ...
  // ---- Auto-brightness status/toggle (arch-on-pro's ALS daemon) ----

  FileView {
    id: autoBrightnessStatusFile
    path: (Quickshell.env("XDG_RUNTIME_DIR") || "/tmp") + "/auto-brightness-status.json"
    watchChanges: true
    printErrors: false
    onLoaded: {
      try {
        var data = JSON.parse(text())
        root.autoBrightnessLux = typeof data.lux === "number" ? data.lux : -1
        root.autoBrightnessPaused = data.manual_override === true
        root.autoBrightnessTs = typeof data.ts === "number" ? data.ts : 0
      } catch (e) {
        root.autoBrightnessTs = 0
      }
      root.refreshAutoBrightnessLive()
    }
    onLoadFailed: {
      root.autoBrightnessTs = 0
      root.refreshAutoBrightnessLive()
  ...
  // Actively re-read every second rather than relying solely on
  // watchChanges — the daemon rewrites this file in place (truncate+write)
  // about once a second, and that pattern isn't guaranteed to fire a fresh
  // change event every single time. Polling is what actually keeps
  // autoBrightnessTs (and therefore autoBrightnessLive) accurate; it also
  // catches a daemon that died without cleaning up (e.g. SIGKILL).
  Timer {
    interval: 400
    running: true
    repeat: true
    onTriggered: {
      autoBrightnessStatusFile.reload()
      root.refreshAutoBrightnessLive()
    }
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Item {
            visible: root.brightnessAvailable
            width: parent.width
            implicitHeight: Math.max(autoBrightnessHeader.implicitHeight, autoBrightnessSwitch.implicitHeight)

            PanelSectionHeader {
              id: autoBrightnessHeader
            ...
            Text {
              id: autoBrightnessStatus
              text: {
                if (root.autoBrightnessBusy) return "Working…"
                if (!root.autoBrightnessLive) return "Off"
                if (root.autoBrightnessPaused) return "Paused"
                return root.autoBrightnessLux >= 0 ? root.autoBrightnessLux + " lux" : "Active"
              }
              color: Qt.darker(root.bar.foreground, 1.4)
              font.family: root.bar.fontFamily
              font.pixelSize: Style.font.caption
              font.bold: true
              anchors.right: autoBrightnessSwitch.left
              anchors.rightMargin: Style.space(8)
              anchors.verticalCenter: parent.verticalCenter

              PanelToolTip {
                visible: autoBrightnessStatusHover.hovered && root.autoBrightnessPaused
                text: "Click to resume now"
                fontFamily: root.bar.fontFamily
              }
              HoverHandler { id: autoBrightnessStatusHover }
            }

            ToggleSwitch {
              id: autoBrightnessSwitch
              checked: root.autoBrightnessEngaged
              foreground: root.bar.foreground
              anchors.right: parent.right
              anchors.verticalCenter: parent.verticalCenter
